...
mod.tcgaSitesVolumeClustHetPFS <- coxph(Surv(dataTCGA$PFS, dataTCGA$Recurrence)~
                                          Sites+Volume+ClusterIDISTH+ClusterIDTextures,data=dataTCGA)
...
legend(x="topright", .6, c("Num Sites <= 7", "Num Sites > 7"), 
...
legend(x="topright", .6, c("Num Sites <= 4", "Num Sites > 4"), 
       col=c('royalblue3', 'orangered'), 
       cex=1.5, y.intersp=1.0, lty=1:2, lwd=2, bty="n")
